...
def main():
    ...
    print(len(list(combinations(routes, 2))), 'combinations')

    aa = set([valves['AA']])
    best_combined_release = 0
    for r1, r2 in combinations(routes, 2):
        if r1.valves_set & r2.valves_set == aa:
            combined_release = r1.release + r2.release
            if combined_release > best_combined_release:
                best_combined_release = combined_release
    print(best_combined_release)
