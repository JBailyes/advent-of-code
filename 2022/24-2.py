...
def main():
    ...
    def travel_between(start:Pos, goal:Pos):
        minute = 1
        routes:list[list[Pos]] = [[start]]
        while True:
            ...
            for route in routes:
                ...
                for p in expedition.surrounding():
                    if p == goal:
                        print('Got to goal at minute', minute)
                        return minute
    ...
    one = travel_between(entrance, way_out)
    two = travel_between(way_out, entrance)
    three = travel_between(entrance, way_out)
